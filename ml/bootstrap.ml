module Plocaml = struct
  ...
  module SPI = struct
    ...
    let execute_plan (_p : plan) (_args : datum array) : spi_result =
      failwith "not implemented"
  ...
  let subtransaction (_f : unit -> 'a) : 'a = failwith "not implemented"
...
let execute_inline (source_text : string) : unit =
  ...
  let lexbuf = Lexing.from_string (source_text ^ "\n;;") in
  let phrases = ref [] in
  (try
     let rec loop () =
...
let () = Callback.register "plocaml_execute" execute_inline
